...
- `reading_game_menu_rollback.png` — доступный «Откат» в обычном Игровом меню; действие возвращает к предыдущей стабильной строке без остаточного меню. В Replay действие скрыто, а при отсутствии предыдущего checkpoint используется существующее disabled-состояние кнопки.
...
- `reading_backlog.png` — прокрученная История с читаемыми русскими репликами и повествованием без square glyphs.
