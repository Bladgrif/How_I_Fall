...
- `game_menu_root.png` — левое slide-in Игровое меню поверх приглушённой, но видимой сцены: список системных действий, разделитель и «Вернуться в игру» (default focus с правым красным маркером) сидят с явными боковыми и вертикальными отступами внутри тёмной панели навигации; пункта отката в меню больше нет.
- `reading_game_menu_1280.png` — та же иерархия и containment при 1280×720.
...
- `game_menu_save_load_return_focus.png` — возврат из встроенного Save/Load: дефолт-фокус «Вернуться» восстановлен ровно с одним маркером; сессионный снапшот доступности отката больше не нужен, потому что кнопки отката в меню нет.

Game menu cleanup + quick rollback pass 2026-09-21: боковые/вертикальные отступы навигации расширены (28px по бокам), rollback убран из бокового меню и добавлен кнопкой «Назад» в Quick Menu (колесо мыши не менялось). `game_menu_root.png` добавлен, `reading_game_menu_rollback.png` удалён (устаревший контракт), `reading_game_menu_1280.png`, `game_menu_main_confirmation.png`, `game_menu_save_load_return_focus.png`, `reading_standard.png`, `reading_quick_menu_hover.png` пересняты, добавлены `reading_quick_menu_rollback.png` и `reading_quick_menu_rollback_1280.png`. Полный PlayerUi graphical E2E снова проходит целиком: починен разрыв стадий hover — кейс `CaptureQuickMenuHoverSettled` отсутствовал в диспетчере с `d61ba76`. Это REVIEW CANDIDATE, не субъективное visual approval.

Outer containment pass 2026-09-21: левый background wash, Header и Navigation игрового меню теперь завершаются на одном общем правом крае (единый geometry-контракт в `VNGameMenuView`), без ~20px перехлёста колонки при 1920×1080. Пересняты по focused GameMenu graphical E2E (`playerPrefsRestored=true`): `game_menu_root.png`, `reading_game_menu_1280.png`, `game_menu_embedded_load.png`, `game_menu_embedded_save_1280.png`, `game_menu_save_load_return_focus.png`. `game_menu_main_confirmation.png` не менялся: колонка меню в нём скрыта dimmer-подтверждением, а fresh GameMenu-сценарий это состояние не снимает. Ширина/вес колонки, отступы кнопок 28px и Save/Load-хост не менялись. Это REVIEW CANDIDATE, не субъективное visual approval.
