...
- `reading_game_menu_rollback.png` — левое slide-in Игровое меню поверх приглушённой, но видимой сцены; доступный откат носит подпись «Назад» и сохраняет прежний контракт.
- `reading_game_menu_1280.png` — та же иерархия и доступный «Назад» (откат) при 1280×720.
...
- `reading_standard.png` — центрированное floating-чтение: текст лежит поверх сцены, поле чтения выровнено по горизонтальной оси, текст внутри поля остаётся выключен влево, а вместо постоянной карточки остаётся только мягкий локальный градиент и низковесное `История / Пропуск / Авто / Быстр. сох.` по нижнему центру.
