...
| ID | Feature | Eternum / edge / input | HIF value / why | HIF / gap | Dep | Size | Risk | Old |
|---|---|---|---|---|---|---|---|---|
| P01 | Image-map hotspots | `imagemap` defines ground/idle/hover and coordinate hotspots that jump or mutate state. | REQUIRED for authored exploration. | DONE — normalized clickable background/image hotspots with hover/focus/click interaction and responsive aspect-fit alignment. | Hotspot component | Medium | Medium | COVERED |
| P02 | Conditional hotspot | Hotspots appear only after flags/items/round conditions. | REQUIRED for authored investigation. | DONE — local prerequisites plus optional existing canonical `GameState` conditions/outcomes; TECH fixtures do not use canonical stats. | Typed conditions | Medium | High | NEW |
| P03 | Object interaction | Evidence, safes, doors, camera buttons and other objects perform actions without leaving the visual scene. | REQUIRED for authored exploration. | DONE — local ephemeral completed state and one-shot hotspot behavior are supported. | Interaction action model | Medium | Medium | NEW |
| P04 | Interaction hover feedback | Dedicated hover image and sound signal clickability. | REQUIRED if hotspots ship. | DONE — hover/focus/click interaction is available; authored sound is not implied. | Hotspot accessibility | Small | Low | NEW |
| P05 | Modal exploration scene | Crime/survival screens are modal and route among locations until objective state changes. | REQUIRED for an authored scene. | DONE — Game Menu remains available during Interactive Scene; Save/Load is blocked only while it is active, and ordinary dialogue/Game Menu/Save-Load flow restores after exit. Active Interactive Scene is intentionally not serialized or restored through Continue/Load; active-route persistence is not supported. | Scene-mode coordinator | Large | High | NEW |
| P06 | Media inspection/zoom | Chat images expand fullscreen; `zoom_image` supports pan/zoom but appears development-oriented and has debug text. `chat.rpy:240-261`, `image_zoom.rpy`. | USEFUL for clues later. | PARTIAL - R06 covers the narrow Chat Image viewer only; pan/zoom remains out of scope. | Media viewer | Medium | Medium | NEW |
...
| ID | Feature | Eternum / edge / input | HIF value / why | HIF / gap | Dep | Size | Risk | Old |
|---|---|---|---|---|---|---|---|---|
| R01 | Message timeline | Chat builds a scrollable history of MC/NPC bubbles and auto-scrolls after replies. `chat.rpy:121-153,313-379`. | USEFUL for teenage drama. | DONE (technical foundation + UI polish) - transient in-memory responsive left Incoming/right Player bubbles, short transcript bottom alignment and long-scroll behavior; no save/backlog/read-history persistence. | `ChatSceneData`, `ChatController` | Large | Medium | COVERED |
| R02 | Player replies | Valid replies appear as buttons and return selected step. | USEFUL. | DONE (technical foundation + UI polish) - exactly two typed options render as persistent bottom reply cards; hidden unavailable responses, typed branch/fallback and exactly-once return remain intact. | Typed choice conditions | Medium | Medium | EXPANDED |
| R03 | Conditional messages | Message `condition` strings choose valid NPC/player branches. | USEFUL behavior, unsafe implementation. | DONE (technical foundation) - existing typed numeric `ChoiceCondition` uses AND semantics; never `eval`. | Condition engine | Medium | High | NEW |
| R04 | Message actions | Selected message executes variable changes/notifications from code strings. | USEFUL behavior. | DONE (technical foundation) - typed `ChatGameStateDelta` maps only the nine saved numeric `GameState` fields; invalid targets apply no effect; never `exec`. | Effect engine | Medium | High | NEW |
| R05 | Typing indicator/delay | NPC typing indicator animates before a delayed response; short pauses separate consecutive messages. `chat.rpy:192-213,388-401`. | USEFUL pacing. | DONE - `ChatEntryPacing` is closed (`Immediate`, `Delay`, `IncomingTyping`) with authored `pacingSeconds`, unscaled deterministic `WaitingForEntryReveal`, exactly-once reveals and transient incoming typing excluded from transcript/backlog/read-history/save. Choice pacing remains Immediate; cleanup cancels pending reveal. `test_chat_v1`: IncomingTyping 1.5s, Image Delay 0.6s, Choice Immediate. `CreateOrRepair` preserves these values. Manual Play Mode QA PASS. | Chat timeline | Small | Low | NEW |
| R06 | Embedded media | Messages can include images; click opens modal fullscreen viewer. | DONE for the approved narrow Chat contract. | DONE - revealed Image with Sprite opens a `ChatController`-owned local aspect-fit overlay; no crop/stretch, no new manager/singleton/lease, no PhoneShell layout change. X/Escape/scrim close, image click consumes, Phone input is blocked, local timers pause without `Time.timeScale`, state/SFX counters remain unchanged, cleanup is safe and state is runtime-only. TECH DEMO ONLY / NOT CANON. Manual QA PASS. Feature `b2477d392d1816b983f33ee9f42825f15506e84a`; `SaveData` v3 and `VNPrototype.unity` unchanged. | Media viewer | Medium | Medium | NEW |
| R07 | Contact identity | Chat data provides background, NPC thumbnail/name; messages align by sender. | REQUIRED if chat exists. | DONE (technical foundation) - typed contact display name and sender alignment; no CharacterData registry or canon contacts. | `ChatSceneData` | Medium | Medium | NEW |
| R08 | Chat notification sound | Opening and incoming messages have separate SFX. | USEFUL. | DONE - typed `ChatSceneData.openSfx` / `incomingSfx`; null is intentionally silent. Existing `AudioManager.PlaySfx(AudioClip)` is reused with existing SFX volume, with no new manager/source family. Open requests once only after successful chat start; Incoming Text/Image requests once at actual reveal, never for typing, Player reply, Choice, refresh, return or cancelled pending entry. `test_chat_v1` is TECH DEMO ONLY / NOT CANON with approved `HIF_TECH_phone_open.wav` / `HIF_TECH_chat_incoming.wav`; its demo expects one open and two incoming cues. `CreateOrRepair` preserves both refs and 1.5s / 0.6s / Immediate timing. Manual audio/pacing QA PASS; `SaveData` v3, `BlockingExclusive` and `VNPrototype.unity` unchanged. | AudioManager | Small | Low | NEW |
...
| ID | Feature | Eternum / edge / input | HIF value / why | HIF / gap | Dep | Size | Risk | Old |
|---|---|---|---|---|---|---|---|---|
| T01 | Single-target timed QTE | Click a highlighted target before timer expires; bar visualizes remaining time. `screens.rpy:2632-2676`. | LATER for rare tension peak. | DONE (technical foundation) - one action button, explicit remaining time/progress and unscaled timeout; manual graphical success/timeout QA passed. TECH DEMO ONLY / NOT CANON; not a reusable full QTE framework. | `TimedNarrativeBeatController` | Medium | Medium | COVERED |
| T02 | Success/failure branch | Target jumps to success; timeout jumps to fail/death/alternate label. | REQUIRED if QTE ships. | DONE (technical foundation) - click and timeout use the existing typed VN scene transition path exactly once after lease exit; `TEST: success` / `TEST: timeout` visibly display and the resolution input does not consume the result scene. | `VNDialogueController.TryRouteToScene` | Medium | High | COVERED |
...
Eternum remains a behavior reference only. How I Fall now has neutral Gallery / Replay, Character Hub / Bios and Chat / Phone technical foundations with Phone UI polish, R05 typed pacing, R06 embedded media viewer and R08 chat notification sounds complete. Chat uses typed `ChatSceneData` Text/Image/Choice entries, existing typed conditions, typed nine-field effects, transient transcript, a responsive portrait clean-glass/smartphone messenger shell, `BlockingExclusive`, Replay denial, generic SaveManager backend protection and exactly-once return. The VN background dims while the ordinary dialogue shell is suppressed with ownership-safe restoration; Incoming bubbles are left, Player bubbles right, the neutral image is an in-transcript media card, and reply cards persist at the bottom. `ChatEntryPacing` uses authored `pacingSeconds` and an unscaled deterministic `WaitingForEntryReveal`: IncomingTyping displays a transient left indicator outside transcript/backlog/read-history/save, delayed entries reveal exactly once, replies remain unavailable during pacing, and cleanup cancels pending reveal. Choice pacing remains Immediate. Short transcripts bottom-align without excess empty space, long transcripts scroll, and layout rebuilds occur on transcript updates rather than every frame. `SaveData` remains v3 and `VNPrototype.unity` is intentionally unchanged. `test_chat_v1`, TEST CONTACT, the placeholder image and `chat_demo_return` are TECH DEMO ONLY / NOT CANON. Terminal replies require one logical click: the earlier perceived second click was Unity Editor/Game View lag at 4K, not a runtime Chat limitation. Manual Chat QA passed at 1280x720, 1920x1080, 2560x1440 and 3840x2160. Chat Phone, Hide UI, Quick Menu, Special Mode, full CI, project validator and scene validation passed with `missingScripts=0` and `invalidEvents=0` on `b2477d392d1816b983f33ee9f42825f15506e84a` in Unity 6000.5.7f1. `SaveData` remains v3, `VNPrototype.unity` remains unchanged, and the technical asset builder preserves IncomingTyping 1.5s / Image Delay 0.6s / Choice Immediate plus approved R08 SFX refs. `ChatSceneData` uses typed `openSfx` / `incomingSfx`; null is silent, the existing AudioManager SFX volume path is reused, successful open requests one cue and actual Incoming Text/Image reveals request two demo cues without typing, reply, refresh, cleanup or return duplicates. Manual audio/pacing QA PASS. R06 adds only the revealed-image local viewer: aspect-fit/no crop/no stretch, X/Escape/scrim close, image-click consume, local timer pause, no viewer SFX and runtime-only cleanup; manual R06 QA and repeated audio QA PASS. The TECH SFX importer issue was malformed `.meta` YAML termination, repaired only by restoring the final newline with GUID/import settings/WAV contents unchanged.

## Maintenance log
- `2026-08-14` — Interactive Hotspot Foundation runtime facts confirmed at `118648baae2c87fb242daa10d0b581218ac07556`: normalized background/image hotspots, hover/focus/click interaction, local ephemeral completion/prerequisites, one-shot behavior, optional canonical `GameState` conditions/outcomes and responsive aspect-fit alignment. Game Menu remains available during Interactive Scene; Save/Load is blocked only while active; ordinary flow restores after exit. Active Interactive Scene is intentionally not serialized or restored through Continue/Load, so active-route persistence is not supported.

- `b2477d392d1816b983f33ee9f42825f15506e84a` - R06 Embedded Chat Media Viewer: revealed Image cards open a local `ChatController` sub-modal overlay with aspect-fit/no crop/no stretch; no new manager, singleton or lease, and PhoneShell ownership is unchanged. X/Escape/scrim close, image click consumes, Phone input is blocked and Escape precedence exists only while viewer active. Local pacing/terminal timers pause without `Time.timeScale`; open/close preserves transcript, GameState, replies and route, adds no SFX and leaves R08 counters unchanged. Cleanup removes the runtime-only overlay; SaveData remains v3 and VNPrototype is unchanged. Manual R06 QA and repeated R08 audio QA PASS. The approved TECH SFX initially failed import because `Technical.meta` and both WAV `.meta` files lacked final newline/valid YAML termination; only the terminator was restored, with GUID/import settings/WAV content unchanged. Importer issue only, not runtime semantics.
- `cc57c08845e94092c4b30be3c15b51199da97e42` - R08 Chat notification sound: typed `AudioClip` `openSfx` / `incomingSfx` are added to `ChatSceneData`; null clips are silent and existing `AudioManager.PlaySfx(AudioClip)` plus SFX volume is reused without a new manager or source family. Open is requested once only after successful chat start. Incoming is requested once only for an actual Incoming Text/Image reveal, never for typing, Player reply, Choice, refresh, cleanup-pending entry or return. `test_chat_v1` is TECH DEMO ONLY / NOT CANON with approved `HIF_TECH_phone_open.wav` and `HIF_TECH_chat_incoming.wav`, IncomingTyping 1.5s, Image Delay 0.6s and Choice Immediate; it expects one open and two incoming cues. `CreateOrRepair` preserves timing and refs. Manual audio/pacing QA PASS; Chat Phone, full CI, project validator and scene validation passed with `missingScripts=0` and `invalidEvents=0`. `SaveData` remains v3; `BlockingExclusive` and `VNPrototype.unity` are unchanged.
- `164d58180bc6a1625ba18ee65de7b567672d816f` - R05 Typed Chat Pacing: closed `ChatEntryPacing` (`Immediate`, `Delay`, `IncomingTyping`) with authored `pacingSeconds`, unscaled deterministic `WaitingForEntryReveal` and exactly-once delayed reveals. IncomingTyping uses a transient left indicator that never enters transcript/backlog/read-history/save; reply cards remain unavailable during pacing, Choice remains Immediate, and cleanup cancels pending reveal. `test_chat_v1` is TECH DEMO ONLY / NOT CANON with IncomingTyping 0.8s, Image Delay 0.35s and Choice Immediate. `CreateOrRepair` now preserves pacing fields; the prior manual QA failure came from builder serialization defaults, not runtime countdown. Manual Play Mode QA PASS; Chat Phone, Special Mode, Hide UI, Quick Menu, Auto, Skip, full CI, validator and scene validation passed with `missingScripts=0` and `invalidEvents=0`. `SaveData` remains v3; `VNPrototype.unity` is unchanged.
- `be2acfff6696f98b03532f5aa20fe3445c71dcd4` - Phone UI polish: approved hybrid clean-glass/smartphone messenger UI is implemented as a responsive portrait runtime `PhoneShell` with dimmed VN background, ownership-safe dialogue-shell suppression, left Incoming/right Player bubbles, neutral media card and persistent bottom reply cards. Selection disables cards immediately and appends one outgoing bubble; short transcripts bottom-align without excess empty space and long transcripts remain scrollable, with layout rebuild only on transcript updates. Terminal reply needs one logical click; the earlier apparent second-click was Unity Editor/Game View lag at 4K, not a runtime Chat bug. Typed data, `BlockingExclusive`, SaveManager backend guard, Replay denial and `SaveData` v3 are unchanged; `VNPrototype.unity` remains unchanged and TEST content is TECH DEMO ONLY / NOT CANON. Manual graphical QA passed at 1280x720, 1920x1080, 2560x1440 and 3840x2160. Chat Phone, Hide UI, Quick Menu, Special Mode, full CI, project validator and scene validation passed in Unity 6000.5.7f1 with `missingScripts=0` and `invalidEvents=0`.
- `f8efadc6f07f3c1991794d7efa1537aa4589d653` - Chat / Phone technical foundation: typed `ChatSceneData` Text/Image/Choice, stable entry IDs, existing typed numeric conditions and typed nine-field effects; a runtime technical UI from narrow Resources config with transient-only transcript. `ChatController` holds `BlockingExclusive`, denies Replay, releases its lease before an exactly-once registered VN return; generic SaveManager backend guard denies save/load during active BlockingExclusive. `SaveData` remains v3; `VNPrototype.unity` was not changed. TEST content is TECH DEMO ONLY / NOT CANON. Manual Save graphical E2E, Save Backend graphical E2E, focused smoke, full CI, validator and scene validation passed. Phone UI polish closure and four-resolution Chat QA are recorded in `be2acfff6696f98b03532f5aa20fe3445c71dcd4`.
...
- `75814acb12d27e733b49102050a21d65895eb6c1` - Timed Narrative Beat QA closure: manual graphical success and timeout paths passed; `TEST: success` / `TEST: timeout` visibly display, terminal fallback no longer has mojibake, and the resolution input does not consume the result scene. Focused smoke, full CI, project validator and scene validation passed in Unity 6000.5.7f1. `SaveData` remains v3; TEST fixtures are TECH DEMO ONLY / NOT CANON.
