...
| ID | Feature | Eternum / edge / input | HIF value / why | HIF / gap | Dep | Size | Risk | Old |
|---|---|---|---|---|---|---|---|---|
| R01 | Message timeline | Chat builds a scrollable history of MC/NPC bubbles and auto-scrolls after replies. `chat.rpy:121-153,313-379`. | USEFUL for teenage drama. | DONE (technical foundation) - transient in-memory Incoming/Player bubbles in runtime technical UI; no save/backlog/read-history persistence. | `ChatSceneData`, `ChatController` | Large | Medium | COVERED |
| R02 | Player replies | Valid replies appear as buttons and return selected step. | USEFUL. | DONE (technical foundation) - exactly two typed options, hidden unavailable responses, typed branch/fallback and exactly-once return. | Typed choice conditions | Medium | Medium | EXPANDED |
...
| R06 | Embedded media | Messages can include images; click opens modal fullscreen viewer. | LATER. | PARTIAL - V1 validates and renders one neutral technical image placeholder; fullscreen viewer is deferred. | Media viewer | Medium | Medium | NEW |
...
Eternum remains a behavior reference only. How I Fall now has neutral Gallery / Replay, Character Hub / Bios and Chat / Phone technical foundations. Chat uses typed `ChatSceneData` Text/Image/Choice entries, existing typed conditions, typed nine-field effects, transient transcript, runtime technical UI, `BlockingExclusive`, Replay denial, generic SaveManager backend protection and exactly-once return. `SaveData` remains v3 and `VNPrototype.unity` is intentionally unchanged. `test_chat_v1`, TEST CONTACT, the placeholder image and `chat_demo_return` are TECH DEMO ONLY / NOT CANON; authored content and final Phone UI polish remain deferred. Manual Chat QA at 1920x1080 passed functionally; additional resolution QA is pending/non-blocking. Known limitation: terminal reply transition can feel like it requires a second click in the current technical UI, although the return route completes automatically and exactly once. Deferred to final Phone UI polish. Full CI/validator/scene validation and both graphical Save E2E suites passed on `f8efadc6f07f3c1991794d7efa1537aa4589d653` in Unity 6000.5.7f1.

## Maintenance log

- `f8efadc6f07f3c1991794d7efa1537aa4589d653` - Chat / Phone technical foundation: typed `ChatSceneData` Text/Image/Choice, stable entry IDs, existing typed numeric conditions and typed nine-field effects; a runtime technical UI from narrow Resources config with transient-only transcript. `ChatController` holds `BlockingExclusive`, denies Replay, releases its lease before an exactly-once registered VN return; generic SaveManager backend guard denies save/load during active BlockingExclusive. `SaveData` remains v3; `VNPrototype.unity` was not changed. TEST content is TECH DEMO ONLY / NOT CANON. Manual Chat QA at 1920x1080 passed functionally; Manual Save graphical E2E, Save Backend graphical E2E, focused smoke, full CI, validator and scene validation passed. Additional resolution QA is pending/non-blocking. Known limitation: terminal reply transition can feel like it requires a second click in the current technical UI, although the return route completes automatically and exactly once. Deferred to final Phone UI polish.
