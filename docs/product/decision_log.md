...
## 2026-09-07 — Ограниченная коррекция общих настроек

**Решение пользователя:** один `SharedPreferencesView`, слева категории Экран / Звук / Текст / Игра, справа только активная категория. Существующие 13 player-facing настроек, диапазоны, `SettingsManager`, service API и PlayerPrefs keys сохраняются; Save/Load не входит в проход.

`PreferencesController` держит черновик до закрытия. `ПРИМЕНИТЬ` записывает только изменённые поля и оставляет экран открытым; при отсутствии изменений кнопка недоступна. `НАЗАД` / Esc отбрасывает только изменения после последнего Apply без подтверждения. `СБРОСИТЬ` выставляет defaults видимых настроек в черновике, не затрагивая скрытые compatibility-поля. Повторное открытие читает текущие committed settings.
