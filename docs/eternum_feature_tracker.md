...
| 11 | Chat / Phone | Typed `ChatSceneData`, Text/Image/Choice, typed condition/effect mapping, transient transcript, BlockingExclusive, exactly-once VN return and responsive runtime phone UI. TEST only; `VNPrototype.unity` unchanged. | narrow Resources bootstrap, existing VN route and SaveManager guard | Medium | High | **DONE (technical foundation + UI polish)** |
