...
| 11 | Chat / Phone | Typed `ChatSceneData`, Text/Image/Choice, typed condition/effect mapping, transient transcript, BlockingExclusive, exactly-once VN return, responsive runtime phone UI and typed R08 open/incoming SFX. TEST only; `VNPrototype.unity` unchanged. | narrow Resources bootstrap, existing VN route, `AudioManager.PlaySfx`, and SaveManager guard | Medium | High | **DONE (technical foundation + UI polish + R08)** |
